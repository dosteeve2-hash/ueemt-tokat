...
export default function HomePage() {
  ...
  return (
    <>
      ...
      >
        {heroImage && (
          <div className="absolute inset-0" style={{ background: 'rgba(20, 164, 77, 0.75)' }} />
        )}
        {!heroImage && (
          <div
      ...
      <section className="py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold mt-2 text-gray-900">Notre Bureau Exécutif</h2>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            {BUREAU_MEMBERS.map(({ prenom, nom, role }) => (
              <div key={nom} className="bg-white rounded-2xl border-2 border-green-100 hover:border-green-400 p-5 text-center transition-all hover:shadow-md group">
                <div className="w-14 h-14 rounded-full bg-green-600 border-2 border-green-200 flex items-center justify-center text-white font-black text-lg mx-auto mb-3 group-hover:scale-105 transition-transform">
                  {prenom[0]}{nom[0]}
                </div>
                <p className="font-bold text-gray-900 text-sm leading-tight">{prenom}</p>
